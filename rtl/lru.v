module lru #(
    ...
    input wire [ADDRESS_SIZE-1:0] address, // Se não for usado aqui, pode ser removido
    ...
    generate
        ...
        for(i = 0; i < ASSOCIATIVITY; i=i+1) begin : matrix_row_gen
            for(j = 0; j < ASSOCIATIVITY; j=j+1) begin : matrix_col_gen
                if(i==j) begin
                    assign matrix_reference[i][j] = 1'b0;
                end
                else if(i>j) begin
    ...
    always @(posedge clock) begin : update
        integer x, y, z;
        // Atualiza se houver um Hit OU se houver um Miss com escrita/acesso habilitado
